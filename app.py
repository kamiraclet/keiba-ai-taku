...
# --- 外部読み込み用関数 ---
def load_markdown(file_path, default_text="準備中..."):
...
<style>
    ...
    .main .block-container { max-width: 1000px; padding-top: 2rem; margin: auto; }
    .title-container { text-align: center; padding-bottom: 2rem; border-bottom: 2px solid #f0f2f6; margin-bottom: 2rem; }
    .stTabs [data-baseweb="tab-list"] { display: flex; justify-content: center; gap: 10px; }
    .stTabs [aria-selected="true"] { background-color: #006400 !important; color: white !important; }
    .custom-footer { margin-top: 5rem; padding: 2rem 0; border-top: 1px solid #eee; text-align: center; color: #999; font-size: 0.8rem; }
</style>
"""
...
if show_policy:
    st.markdown(load_markdown("policy.md", "ポリシー準備中"))
    st.markdown("<br><div style='text-align: center;'><a href='/' target='_self'>← サイトへ戻る</a></div>", unsafe_allow_html=True)
    ...
    with tab1:
        st.markdown(load_markdown("profile.md"))

    with tab2:
        st.markdown(load_markdown("about_ai.md"))
    ...
    with tab4:
        st.markdown(load_markdown("special_feature.md"))

    with tab5:
        st.markdown(load_markdown("venue_data.md"))

    with tab6:
        st.markdown(load_markdown("column.md"))
